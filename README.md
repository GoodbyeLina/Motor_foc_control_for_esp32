...
| 模式 | 命令 | 说明 |
|------|------|------|
| **开环** (`mode 0`) | `mode 0` | 无需编码器和电流采样，直接合成角度，固定 Uq=Vbus/3 |
| **电流闭环** (`mode 1`) | `mode 1` | 单电流环：目标电流 → PID → Uq，适合力矩控制 |
| **速度闭环** (`mode 2`) | `mode 2` | **默认** 双闭环：速度PID + 电流PID，适合速度控制 |
...
1. 烧录后打开 `idf.py monitor`
2. 看到帮助信息后，输入 `v 5` 设置速度
3. 电机应以约 48 RPM 的速度旋转
4. 输入 `s` 停止电机
