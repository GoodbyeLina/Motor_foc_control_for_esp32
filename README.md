...
相比于 SPWM（固定加 $$V_{bus}/2$$ 偏置），SVPWM 动态注入零序分量，使母线电压利用率从 50% 提升到 **57.7%**（$$V_{bus}/\sqrt{3}$$）。

$$\begin{aligned} \text{raw}_a &= \frac{U_\alpha}{V_{bus}} \\ \text{raw}_b &= \frac{-U_\alpha + \sqrt{3} \cdot U_\beta}{2 \cdot V_{bus}} \\ \text{raw}_c &= \frac{-U_\alpha - \sqrt{3} \cdot U_\beta}{2 \cdot V_{bus}} \\ t_{offset} &= 0.5 - 0.5 \cdot (\min(\text{raw}) + \max(\text{raw})) \\ dc_a &= \text{raw}_a + t_{offset} \\ dc_b &= \text{raw}_b + t_{offset} \\ dc_c &= \text{raw}_c + t_{offset} \end{aligned}$$
...
| 项目 | 数值 | 说明 |
|------|------|------|
| 最高转速（速度闭环） | **~58 RPM**（≈ 6.0 rad/s） | 受 SVPWM 电压限幅限制（Uq=7.21V，接近 7.27V 极限） |
| 最高转速（开环） | 取决于目标速度设置 | 开环无反馈，目标速度可高于 58 RPM |
| 速度控制分辨率 | ~1.5 RPM | AS5600 12位编码器量化噪声 |
...
| 项目 | 数值 | 说明 |
|------|------|------|
| 电流环响应 | 有波动，需进一步调参 | P=1.5, I=50 组合存在跟踪不一致问题 |
| 最大输出电压 | **7.27V**（Uq 限幅） | Vbus/√3，SVPWM 最大调制比 |
| 最小启动电流 | **~0.05 A** | 电机刚能转动的电流值 |
| 有效工作范围 | **0.05 ~ 0.50 A** | SVPWM 相比 SPWM 扩大了电压裕量 |
| 调制方式 | **SVPWM（7段对称）** | 从 SPWM 升级，母线利用率 +15.4% |

**电流环阶梯测试数据（SVPWM，12.6V 供电）：**

| 目标电流 | 实测 Iq | Uq | 速度 | 追踪误差 | 状态 |
|---------|---------|----|------|---------|------|
| 0.05 | 0.0577 | 1.01 | 16.1 | +15.4% | ⚠️ 低速噪声影响 |
| 0.10 | 0.1139 | 1.86 | 28.3 | +13.9% | ⚠️ 偏高 |
| 0.15 | **0.1090** | 2.58 | 35.0 | **-27.3%** | ❌ 异常跳变（齿槽转矩？） |
| 0.20 | 0.1704 | 3.14 | 39.0 | -14.8% | ⚠️ 偏低 |
| 0.25 | 0.2422 | 3.60 | 44.0 | -3.1% | ✅ 跟踪良好 |
| 0.30 | 0.2681 | 4.20 | 46.6 | -10.6% | ⚠️ 偏低 |
| 0.35 | 0.3335 | 4.71 | 50.2 | -4.7% | ✅ 跟踪良好 |
| 0.40 | 0.3438 | 5.28 | 51.4 | **-14.1%** | ⚠️ 再次恶化 |
| 0.45 | **0.5315** | 5.56 | 52.6 | **+18.1%** | ❌ 过冲 |
| 0.50 | 0.4772 | 6.25 | 54.1 | -4.6% | ✅ 跟踪良好 |

> **结论：**
> - SVPWM 将 Uq 限幅从 6.3V（SPWM）提升到 **7.27V**，电压裕量增加了 **+15.4%**
> - 但在 0.50A 时 Uq=6.25V，仍远低于 7.27V 极限，**电流跟踪不准确的主因是 PID 参数，而非电压饱和**
> - 0.150A 和 0.400A 的跟踪恶化很可能是积分增益过高（I=50）导致的积分饱和/反复震荡
> - **下阶段建议：** 电流环 PID 改为 P=2.0, I=20, D=0.5, ramp=50 再测

### 11.3 速度环性能

| 项目 | 数值 | 说明 |
|------|------|------|
| 速度稳态精度（20~50 RPM） | **±3%** | 速度环调校效果良好 |
| 稳态误差（全范围） | -16.9% ~ +12.2% | 低速和高速端误差增大 |
| Uq-速度线性度 | R² ≈ 0.998 | 60 RPM 以下近似完美线性 |

**速度环阶梯测试数据（SVPWM，自动测试）：**

| 目标速度 | 实测速度 | 误差% | Iq | Uq | 状态 |
|---------|---------|-------|----|----|------|
| 10 | 11.2 | **+12.2%** | 0.0036 | 0.44 | ⚠️ 低速采样噪声占主导 |
| 20 | 19.4 | -2.8% | 0.0559 | 1.47 | ✅ |
| 30 | 30.3 | +1.0% | 0.0675 | 2.06 | ✅ |
| 40 | 40.1 | +0.3% | 0.2304 | 3.22 | ✅ **最佳点** |
| 50 | 50.6 | +1.1% | 0.3566 | 4.60 | ✅ |
| 60 | 58.0 | **-3.3%** | 0.6150 | **7.16** | ⚠️ 接近饱和（Uq=7.16/7.27=98.5%） |
| 70 | 58.1 | **-16.9%** | 0.6238 | **7.21** | ❌ **完全饱和**（Uq=7.21/7.27=99.2%） |

> **分析：**
> - v=10 误差 +12.2%：Iq=0.0036A 远低于电流采样分辨率，属传感器硬极限
> - v=40 误差仅 +0.3%：速度环最佳工作点
> - v=20~50 区间精度均在 **±3%** 以内，速度环调校效果良好 ✅
> - **60 RPM 时 Uq=7.16V（98.5% 极限）** → 相比 SPWM 的 6.3V 极限，SVPWM 让系统多跑了约 **8 RPM**（+16%）
> - 70 RPM 与 60 RPM 实测速度几乎相同（58.0→58.1），说明已到物理极限

### 11.4 电压-速度特性（SVPWM）

| Uq (V) | 速度 (RPM) | 速度 (rad/s) | 每 RPM 所需电压 |
|--------|-----------|-------------|----------------|
| 0.44 | 10 | 1.04 | 0.044 |
| 1.47 | 19.4 | 2.03 | 0.076 |
| 2.06 | 30.3 | 3.17 | 0.068 |
| 3.22 | 40.1 | 4.20 | 0.080 |
| 4.60 | 50.6 | 5.30 | 0.091 |
| 7.16 | 58.0 | 6.07 | 0.123 |
| 7.21 | 58.1 | 6.08 | 0.124 |

> - 50 RPM 以下 Uq-速度近似线性，斜率约 **0.087 V/RPM**
> - 50→60 RPM 需要额外 **2.56V**（克服 BEMF 非线性增长），仅剩 **2.67V** 裕量
> - **SVPWM 极限 7.27V vs SPWM 极限 6.3V** → SVPWM 提供了额外 0.97V 电压空间，使最高速度从 ~50 RPM 提升到 **58 RPM**（+16%），与理论计算吻合 ✅

### 11.5 性能总结

| 指标 | SPWM（旧） | SVPWM（新） | 改善 |
|------|----------|------------|------|
| 最高速度 | ~50 RPM | **~58 RPM** | **+16%** ✅ |
| 速度稳态精度（20~50 RPM） | ±2% | **±3%** | 持平 ✅ |
| Uq 限幅 | 6.3V | **7.27V** | **+15.4%** ✅ |
| 电流环有效范围 | 0.05 ~ 0.35 A | **0.05 ~ 0.50 A** | 更宽 ✅ |
| Uq 线性工作区 | 0 ~ 60 rad/s | 0 ~ 50 RPM | 基本一致 |
| Uq 饱和点 | ~6.2 V | **~7.2 V** | ✅ 随限幅提升 |
...
| D | 0.0 | 0.0 |
| Ramp | 100000 | 500 |
| Limit | **7.27V**（SVPWM） | 1.0A |
| 抗饱和 | ✅ 已启用 | ✅ 已启用 |
| 模式切换重置 | ✅ 自动重置 | ✅ 自动重置 |
| 调制方式 | **SVPWM 7段对称** | — |

> **⚠️ 待优化：** 电流环 PID 参数（P=1.5, I=50）导致跟踪误差波动大（-27% ~ +18%），建议改为 P=2.0, I=20, D=0.5, ramp=50
